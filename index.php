...
($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $_POST['action'] == 'register') {
        ...
        if ($password !== $confirmPassword) { 
            ...
            // Validate name to allow only Latin 
            // letters
            if (!preg_match('/^[a-zA-Z]+$/', 
            $name)) {
                $errorMessage = "Name should only 
                contain Latin letters.";
            } 
            // Validate UID to accept only 
            // numbers
            elseif (!preg_match('/^\d+$/', $uid)) 
            {
                $errorMessage = "UID should only 
                contain numbers.";
            } else {
                $hashed_password = 
                ...
                ($stmt->num_rows > 0) {
                    ...
                    // Check if UID already 
                    // exists
                    $stmt = 
                    $conn->prepare("SELECT id 
                    FROM users WHERE uid = ?"); 
                    $stmt->bind_param("s", $uid); 
                    $stmt->execute(); 
                    $stmt->store_result(); if 
                    ($stmt->num_rows > 0) {
                        $errorMessage = "This UID 
...
lang="en"> <head>
    ...
    Register</title> <style>
        ...
        margin-bottom: 1.5rem; } h2 { color: 
        #333; font-size: 1.5rem; margin: 1rem 0; 
        }
        label { display: block; margin: 0.5rem 0 
        0.2rem; color: #495057; text-align: left; 
        }
        input[type="text"], input[type="email"], 
        input[type="password"] { width: 100%; 
        padding: 0.8rem; border: 1px solid 
        #ced4da; border-radius: 5px; 
        margin-bottom: 1rem; transition: all 0.3s 
        ease; } input[type="text"]:focus, 
        input[type="email"]:focus, 
        input[type="password"]:focus { 
        border-color: #007bff; box-shadow: 0 0 
        5px rgba(0, 123, 255, 0.3); outline: 
        none; } .button { width: 100%; padding: 
        0.8rem; background-color: #007bff; 
        border: none; color: white; font-size: 
        1rem; border-radius: 5px; cursor: 
        pointer; transition: background-color 
        0.3s; } .button:hover { background-color: 
        #dc3545; }
        .error { color: #dc3545; font-weight: 
        bold; } .success { color: #28a745; 
        font-weight: bold; }
    </style> </head> <body> <div 
    class="container">
        <h1>Login / Register</h1> <div 
